...
class User extends ActiveRecord implements IdentityInterface
{
    ...
    */
    public static function getClientList($addBlank = false)
    {
        $models = User::Find()->where(['role_id' => 3]);
        $gname = function ($c)
        {
            $item = User::findOne(['id' => $c["id"]]);
            return $item->id ." (".$item->user_name.")";
        };

        $items = ArrayHelper::map($models, 'id', $gname);
        if ($addBlank){
            $items["-"] = "";
        }
        natcasesort($items);
    ...
    public static function findByUsername($username)
    {
      ...
      /*AdamistrationSuper_AdminClientGroup*/
    ...
    {   
        ...
        if ($model){
            ...
            exit;*/
                    ...
                    //'role' => self::ROLE_ADMIN, 'role' => self::ROLE_USER]     
                 return true;
          } else {
